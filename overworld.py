...
class Node(pygame.sprite.Sprite):
    ...
    def update(self):
        if self.status == 'available':  # only animate the available unlocked levels
            self.animate()
        else:
            tinted_surface = self.image.copy()
            # makes image black transparent indicating level is locked
...
class Overworld:
    def __init__(self, start_level, max_level, surface, create_level):
        ...
        # start time for overworld
        self.start_time = pygame.time.get_ticks()  # starts when overworld is created
    ...
    def input_timer(self):
        if not self.allow_input:
            ...
            # current_time keeps increasing as game goes on # more info on video at 22:22
    ...
    def run(self):
        ...
        self.sky.draw(self.display_surface)  # draws sky in overworld
        self.draw_paths()
